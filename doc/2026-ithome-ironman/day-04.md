...
一個漏洞被發現後，不會立刻變成公開的 CVE Record。

中間還有通報、確認 scope、判斷是否符合指派規則、保留 CVE ID、協調公開時間、整理必要資料，以及正式發布等步驟。這些工作有時能在短時間內完成，有時則會因為重現、版本確認、修補排程或跨組織協調而持續一段時間。
...
### CVE Record Published 就代表已修補？

不一定。是否已有修補，需要查看 vendor advisory、受影響版本與 reference。

### CVE ID 的年份就是發現年份？

不一定。年份與 ID 保留或漏洞公開的年份有關，不代表發現日期。

### NVD 還沒有評分，代表 CVE 無效？

不是。NVD enrichment 是下游流程，與 CVE Record 是否 Published 是不同狀態。
...
明天會進一步拆解一筆 CVE Record：裡面通常有哪些欄位？哪些是識別資料，哪些用來描述產品、版本、影響與資料來源？
